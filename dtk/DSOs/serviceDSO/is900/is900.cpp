	 ...
	 )
	{
	  ...
	  if(
	     ...
	     )
	    {
	      ...
	      state[j][0] = (getFloat(&ptr[Y]) - 0.0f)/152.4f;
	      state[j][1] = (getFloat(&ptr[X]) - 30.48f)/152.4f;
	      state[j][2] = -(getFloat(&ptr[Z]) + 154.0f)/152.4f;
	  ...
	  // WAND
		  ...
		  )
	    {
	      ...
	      state[j][0] = ( getFloat(&ptr[Y]) - 0.0f)/152.4f;
	      state[j][1] = ( getFloat(&ptr[X]) - 30.48f)/152.4f;
	      state[j][2] = (-getFloat(&ptr[Z]) - 157.0f)/152.4f;
	      state[j][3] = -getFloat(&ptr[H]);
	      state[j][4] =  getFloat(&ptr[P]) - 30.0f;
	      state[j][5] =  getFloat(&ptr[R]);
