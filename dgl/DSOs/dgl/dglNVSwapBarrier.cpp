...
class dglNVSwapBarrier : public dglAugment
{
public:

	dglNVSwapBarrier(dtkManager* m);
	~dglNVSwapBarrier(void) ;
	int initGL(void);
...
dglNVSwapBarrier::dglNVSwapBarrier(dtkManager *m):dglAugment("dglNVSwapBarrier")
{
	m_dpy = NULL;
	m_drawable = 0;
	m_swapgroup = 1;
...
// destructor
dglNVSwapBarrier::~dglNVSwapBarrier(void) 
{
	if( m_drawable && m_dpy )
	{
...
int dglNVSwapBarrier::initGL(void) 
{
	...
	char* extloc = strstr( extensions, "GLX_NV_swap_group" );
	printf( "%s\n\n", extensions );
	if( extloc )
		printf( "%s\n", extloc );
...
{
	return new dglNVSwapBarrier(manager);
}
